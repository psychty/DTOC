library(png); library(gridExtra); library(XLConnectJars); library(XLConnect); library(plyr); library(dplyr);library(tidyr); library(reshape2); library(scales); library(ggplot2); library(xts); library(dygraphs); library(grid); library(htmlwidgets)
...
Combined_Month_persons <- read.csv("./Delayed Transfers of Care/DToC_Persons_Reason_for_Delay_May-2017.csv", header = TRUE, check.names = FALSE)
