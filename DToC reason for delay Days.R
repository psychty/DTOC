...
DToC_LA_ts <- dygraph(DToC_Monthly_LA[,2:13], main = paste("Delayed Transfers of Care (days) - Reason for delay - ", chosen_area, sep = "" ), ylab = "Number of days delayed", xlab = "Month") %>%
  ...
  dyRangeSelector(dateWindow = c("2016-01-01", "2019-05-01"), height = 20, strokeColor = "") %>%
  dyAxis("x", drawGrid = FALSE) %>%
  dyOptions(includeZero = TRUE) %>%
  dyCSS("~/Documents/Repositories/DTOC/dyg.css")
...
DToC_LA_long <- DToC_LA %>% 
  gather(key = "Reason", value = "Days", `Awaiting completion of assessment`:`Housing - Patients not covered by NHS and Community Care Act`)
...
DToC_LA_long <- DToC_LA_long %>% 
  mutate(Reason = sub('(.{1,30})(\\s|$)', '\\1\n', Reason)) %>% 
  mutate(Period = factor(Period, levels = month_order)) %>% 
  arrange(Period)
